...
*Note* that, since [MMMMMMMMMMMM XX, 2022 Release .Stat Suite .NET X.X.X](https://sis-cc.gitlab.io/dotstatsuite-documentation/changelog/#mmmmmmmmmmmmmmmmmm-XX-2022), only dimension values with available data are displayed in the edit filters panel. It means that, if e.g. a dataflow has only annual data but it is using a dimension with more frequency values (e.g. Monthly, or Daily), then only Annual frequency will appear in the Frequency selector.
...
![DLM Excel-Addin data table content](/dotstatsuite-documentation/images/data-table-content.png)
